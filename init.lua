local ImmersiveFirstPerson = { version = "1.3.2" } -- FIX: bump patch
...
-- ==== Helpers ====
local function defaultFovOrNil()
...
local wasReset = true
local function ResetCamera(force)
...
local function IsCrouching()
    ...
    return p and p:GetPS():IsCrouch() or false
end
...
function ImmersiveFirstPerson.RestoreFreeCam()
    ...
    if (curEuler.pitch == 0 and curEuler.roll == 0 and curEuler.yaw == 0)
        and (curPos.x == 0 and curPos.y == 0 and curPos.z == 0) then
        freeLookRestore.progress = 0
        Helpers.SetRestoringCamera(false)
        Helpers.SetFreeObservation(false)
        return
    ...
    local itersWithSpeed = Vars.FREELOOK_SMOOTH_RESTORE_ITERS / (Config.inner.smoothRestoreSpeed or 1) * Vars.FREELOOK_SMOOTH_RESTORE_ITERS
...
-- FIX: локальная easing-функция (если в Easings есть outExpo — можно подменить)
local function easeOutExp(t)
    -- плавно к 1, избегая жёсткого хвоста
...
function ImmersiveFirstPerson.HandleFreeLook(relX, relY)
    ...
    local xSensitivity = 0.07 / zoom * (Config.inner.freeLookSensitivity or 20) / 20
    local ySensitivity = 0.07 / zoom * (Config.inner.freeLookSensitivity or 20) / 20

    local yaw   = CalculateYaw(curYaw, relX, xSensitivity)
    local pitch = CalculatePitch(curPitch, relY, weapon, ySensitivity) -- FIX: проброс sens

    -- вычислим прогресс по тангажу (для FOV и shift-ов)
    local maxPitchAbs = weapon and (Vars.FREELOOK_MAX_PITCH_COMBAT_UP) or (Vars.FREELOOK_MAX_PITCH)
    local pitchProgress = math.min(1, math.max(0, -pitch / maxPitchAbs)) -- 0..1, когда смотрим вниз
    ...
    local y = CalculateY(pitch, weapon, xShiftMultiplier)
    local z = CalculateZ(pitch, weapon, xShiftMultiplier)

    local fov = CalculateFOV(pitchProgress) -- FIX: проброс pitchProgress
...
function CalculateYaw(curYaw, relX, xSensitivity)
    ...
    local base = math.abs(curYaw / Vars.FREELOOK_MAX_YAW)
    local yawProgress = (yawingOut and easeOutExp(base) or 0) + (1 - easeOutExp(base))
    local nextYaw = curYaw - (relX * xSensitivity * yawProgress)
    return math.min(Vars.FREELOOK_MAX_YAW, math.max(-Vars.FREELOOK_MAX_YAW, nextYaw))
end

function CalculatePitch(curPitch, relY, weapon, ySensitivity) -- FIX: + ySensitivity
    local maxPitch = weapon and Vars.FREELOOK_MAX_PITCH_COMBAT_UP or Vars.FREELOOK_MAX_PITCH
    local nextPitch = curPitch + (relY * ySensitivity)
    return math.min(maxPitch, math.max(-maxPitch, nextPitch))
end
...
function CalculateFOV(pitchProgress) -- FIX: принимает аргумент
...
function ImmersiveFirstPerson.Init()
    ...
    registerForEvent("onDraw", function()
        ...
        local changed = false -- FIX: объявление
        Config.inner.freeLookSensitivity, changed = ImGui.SliderInt("Sensitivity", math.floor(Config.inner.freeLookSensitivity or 20), 1, 100)
        if changed then
            Config.SaveConfig()
        end
